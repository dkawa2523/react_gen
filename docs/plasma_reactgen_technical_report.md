...
> **注記 (2026-08-16):** 6章のレイヤ構成と7章のコマンド名は旧実装のものです。
> 現在の構成は [design](core_design.md) を参照してください。1〜5章の物理と手法、
> 8章のベンチマーク結果は有効です。コマンドは `reactgen` から `rgen` / `acquire`
> に、`enrich`/`promote` は `rgen ingest` とオーバーレイに置き換わりました。

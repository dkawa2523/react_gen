...
def build_species_lineage_dot(
    dataset: VisualizationDataset,
    *,
    options: GraphvizOptions,
) -> str:
    states_by_id = {str(state.get("id")): state for state in dataset.states}
    edges = _lineage_edges(dataset.reactions, states_by_id, options)
    ...
    return render_species_graph(
        graph_name="species_lineage",
        title=f"Species lineage: {case_name}",
...
def _lineage_edges(
    ...
    for reaction in reactions:
        if options.max_reactions is not None and considered >= options.max_reactions:
            break
        added = _introduced_edges(reaction, states_by_id, options)
        edges.extend(added)
        considered += bool(added)
    return edges


def _introduced_edges(
    ...
    introduced = {
        str(species_id)
        for species_id in reaction.get("introduced_species", [])
        if str(species_id) != "e"
    }
    if not introduced:
        return []
    ...
    return [
        (
            source,
            target,
            reaction_edge_attributes(reaction, penwidth="1.8"),
        )
        for source, target in mapped
        if target in introduced and (options.include_self_loops or source != target)
    ]


__all__ = ["build_species_lineage_dot"]
